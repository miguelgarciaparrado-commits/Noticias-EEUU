...
def process_price_alerts(assets_map):
    """Comprueba alertas de precio y envía WhatsApp si se cumplen."""
    alerts = supabase.table('price_alerts').select('*').eq('active', True).execute().data or []
    if not alerts:
        return
    print(f'\n{len(alerts)} alertas de precio activas')

    # Cache de precio por símbolo (para no llamar 100 veces a la misma API)
    price_cache = {}
    ...
    for alert in alerts:
        ...
        if symbol not in price_cache:
            price_cache[symbol] = get_current_price(asset, None)
        current_price = price_cache[symbol]
        if current_price is None:
            continue

        target = float(alert['target_price'])
        direction = alert.get('direction') or 'any'

        # ¿Hay disparo? Comparamos contra el precio guardado en triggered_price (último checked)
        # Si nunca se ha disparado, comparamos contra triggered_price NULL
        # Pero como el dedup es importante, usamos un margen de tolerancia: si current_price está
        # dentro del 0.05% del target en la dirección correcta, lo consideramos disparado.

        last_seen = alert.get('triggered_price')
        fired = False
        if direction == 'cross_up':
            # Necesitamos precio anterior < target y actual >= target
            # Usamos una columna previously_seen_price si existiera; si no, simplemente
            # disparar si está por encima del objetivo.
            if last_seen is None and current_price >= target:
                fired = True
            elif last_seen is not None and float(last_seen) < target <= current_price:
                fired = True
            ...
            # Si no se ha disparado nunca y el precio toca/cruza el objetivo
            if last_seen is None:
                # Si están dentro de un 0.1% de tolerancia
                tolerance = target * 0.001
                if abs(current_price - target) < tolerance:
                    fired = True
                elif (current_price >= target and last_seen is None):
                    # ya está al otro lado del nivel: disparar una vez
                    fired = True

        if not fired:
            # Actualizamos triggered_price con el último visto, para detectar cruces en próximas runs
            try:
        ...
        # Disparado — buscar perfil y enviar WhatsApp
        prof_resp = supabase.table('profiles').select(
            'wa_phone, wa_apikey'
        ).eq('user_id', alert['user_id']).maybe_single().execute()
        profile = prof_resp.data if prof_resp else None
        if not profile or not profile.get('wa_phone'):
            print(f'  [SKIP] alerta precio #{alert["id"]} sin teléfono')
            continue
        phone = profile['wa_phone']
        apikey = profile.get('wa_apikey') or WA_APIKEY_DEFAULT
        if not apikey:
        ...
        msg = (
            f'{icon} *Alerta de precio*\n\n'
            f'*{display}*\n'
            f'Precio {dir_word} {target:,.{decimals}f}\n'
            f'Precio ahora: *{current_price:,.{decimals}f}*'
        )
        if label:
            msg += f'\n\n_{label}_'
        ...
        if send_whatsapp(phone, apikey, msg):
            print(f'  [OK] alerta precio #{alert["id"]} -> {phone}')
            try:
                # Desactivar la alerta tras dispararla (one-shot por defecto)
                supabase.table('price_alerts').update({
                    'active': False,
            ...
            print(f'  [FAIL] alerta precio #{alert["id"]}')




def process_active_trades(assets_map):
    """Procesa trades activos: SL hit, TP hit, hitos +1R/+1.5R, trailing estructural."""
    try:
        trades = supabase.table('trade_positions').select('*').eq('status', 'active').execute().data or []
    except Exception as e:
        print(f'[ERROR query trades] {e}')
        return
    if not trades:
        return
    print(f'\n{len(trades)} trades activos')
    ...
    for trade in trades:
        ...
        trail_mode = trade.get('trail_mode') or 'structural'
        highest_r = float(trade.get('highest_r_reached') or 0)
        ...
        r_now = move / stop_dist

        # ¿Tocó SL?
        sl_hit = (direction == 'long' and current_price <= current_stop) or                  (direction == 'short' and current_price >= current_stop)
        # ¿Tocó TP?
        tp_hit = (direction == 'long' and current_price >= target) or                  (direction == 'short' and current_price <= target)

        # Profile para WhatsApp
        prof_resp = supabase.table('profiles').select('wa_phone, wa_apikey').eq('user_id', trade['user_id']).maybe_single().execute()
        profile = prof_resp.data if prof_resp else None
        phone = profile.get('wa_phone') if profile else None
        apikey = (profile.get('wa_apikey') if profile else None) or WA_APIKEY_DEFAULT
        decimals = asset.get('decimals') or 2
        display = asset.get('display_name') or symbol

        # ===== CIERRE POR SL =====
        if sl_hit:
            r_result = (current_stop - entry) / stop_dist if direction == 'long' else (entry - current_stop) / stop_dist
            try:
                supabase.table('trade_positions').update({
                    'status': 'sl_hit',
            ...
            if phone and apikey:
                msg = (
                    f"❌ *SL ejecutado* — {direction.upper()} {display}\n\n"
                    f"Entrada: {entry:,.{decimals}f}\n"
                    f"SL: {current_stop:,.{decimals}f}\n"
                    f"Resultado: *{r_result:+.2f}R*\n\n"
                    f"_Stop Loss is Law. Cierra y siguiente._"
                )
                if send_whatsapp(phone, apikey, msg):
                    print(f'  [SL HIT] trade #{trade["id"]} -> {phone}')
                    time.sleep(7)
            continue

        # ===== CIERRE POR TP =====
        if tp_hit:
            r_result = abs(target - entry) / stop_dist
            try:
                supabase.table('trade_positions').update({
                    'status': 'tp_hit',
                    'closed_at': datetime.now(timezone.utc).isoformat(),
            ...
            if phone and apikey:
                msg = (
                    f"🎯 *TP alcanzado* — {direction.upper()} {display}\n\n"
                    f"Entrada: {entry:,.{decimals}f}\n"
                    f"TP: {target:,.{decimals}f}\n"
                    f"Resultado: *+{r_result:.2f}R* ✅\n\n"
                    f"_Disciplina ganadora. Trade cerrado._"
                )
                if send_whatsapp(phone, apikey, msg):
                    print(f'  [TP HIT] trade #{trade["id"]} -> {phone}')
                    time.sleep(7)
            continue

        # ===== TRACKING DE HITOS =====
        # Detectar primer cruce de +1R, +1.5R, +1.8R (alertas educativas)
        update_data = {}
        if r_now > highest_r:
            update_data['highest_r_reached'] = r_now

        # +1R cruzado por primera vez (highest_r anterior < 1, ahora >= 1)
        if highest_r < 1 and r_now >= 1:
            update_data['current_phase'] = 'phase_2'
            if phone and apikey:
                if trail_mode in ('breakeven', 'structural'):
                    msg = (
                        f"🔄 *+1R alcanzado* — {direction.upper()} {display}\n\n"
                        f"Precio: {current_price:,.{decimals}f}\n"
                        f"R actual: *+{r_now:.2f}R*\n\n"
                        f"💡 Considera mover SL a *Break-Even* ({entry:,.{decimals}f})\n"
                        f"A partir de aquí ya no puedes perder."
                    )
                elif trail_mode == 'partial':
                    msg = (
                        f"🔄 *+1R alcanzado* — {direction.upper()} {display}\n\n"
                        f"Precio: {current_price:,.{decimals}f}\n\n"
                        f"💡 Considera *cerrar 50%* y dejar correr el resto con SL en BE."
                    )
                else:
                    msg = (
                        f"🔄 *+1R alcanzado* — {direction.upper()} {display}\n\n"
                        f"Precio: {current_price:,.{decimals}f}\n"
                        f"Modo SL fijo: mantén disciplina hasta TP o SL."
                    )
                if send_whatsapp(phone, apikey, msg):
                    print(f'  [+1R] trade #{trade["id"]}')
                    time.sleep(5)

        # +1.5R cruzado
        elif highest_r < 1.5 and r_now >= 1.5:
            update_data['current_phase'] = 'phase_3'
            if phone and apikey and trail_mode == 'structural':
                msg = (
                    f"🔄 *+1.5R* — {direction.upper()} {display}\n\n"
                    f"Precio: {current_price:,.{decimals}f}\n"
                    f"Si hay nuevo swing {'low' if direction == 'long' else 'high'} estructural, sube el SL ahí."
                )
                if send_whatsapp(phone, apikey, msg):
                    print(f'  [+1.5R] trade #{trade["id"]}')
                    time.sleep(5)

        # +1.8R (cerca del TP)
        elif highest_r < 1.8 and r_now >= 1.8:
            if phone and apikey:
                msg = (
                    f"⚠️ *+{r_now:.2f}R · cerca del TP* — {display}\n\n"
                    f"Precio: {current_price:,.{decimals}f}\n"
                    f"TP: {target:,.{decimals}f}\n\n"
                    f"💡 Ya casi. Mantén disciplina — no cierres antes por miedo."
                )
                if send_whatsapp(phone, apikey, msg):
                    print(f'  [+1.8R] trade #{trade["id"]}')
                    time.sleep(5)

        if update_data:
            try:
                supabase.table('trade_positions').update(update_data).eq('id', trade['id']).execute()
            except Exception as e:
                print(f'  [WARN] {e}')
...
def run():
    ...
    # Procesar alertas de precio
    try:
        process_price_alerts(assets_map)
    except Exception as e:
        print(f'[ERROR price alerts] {e}')

    # Procesar trades activos
    try:
        process_active_trades(assets_map)
    except Exception as e:
        print(f'[ERROR active trades] {e}')
